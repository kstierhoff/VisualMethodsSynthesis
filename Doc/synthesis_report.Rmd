...
boot.data.rov <- obs.rov %>% 
  ...
  mutate(abund  = dens_n*stratum_area, # Biomass (g)
         biom   = dens_g*stratum_area) # Abundance (n)
      ...
      for (k in unique(boot.data.rov$stratum)) {
        ...
        if (nrow(boot.data) > 0) {
          ...
          for (ii in seq(1:B)) {
            ...
            boot.biom[ii]  <- mean(sample(boot.data$biom,nrow(boot.data),replace = T))
          }
...
```{r ALL_bootstrap}
boot.all <- bind_rows(boot.summ.sample.rov,boot.summ.sample.sub) %>% 
  ungroup() %>% 
...
ggplot(filter(boot.all, biom.cv > 0), aes(factor(sciName),platform)) + geom_raster(aes(fill = biom_cv_cut)) + 
  scale_fill_manual(values = c("green","yellow","orange","red")) +
  geom_text(aes(label = round(biom.mean,0))) +
  coord_flip() + 
  facet_wrap(~site, nrow = 1) + theme_bw()

ggsave(here("Figs/fig_boot_cv_raster.png"),height = 7, width = 7)
```
