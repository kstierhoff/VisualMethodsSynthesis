...
# Read good nav IDs
good.rov.nav <- read_csv(here("Data/ROV/good_rov_nav.csv"))$nav_id
...
rov.video_obs <- filter(rov.video_obs,dive_name %in% rov.logs$dive_name) %>% 
  ...
  left_join(select(spp_codes,species_code,sci_name,sciName,commonName,species_group)) %>% 
...
rov.nav.boot <- rov.nav %>% 
  group_by(dive_name,stratum) %>% 
  summarise(
    distance = sum(disp),
    area = sum(area))

rov.nav.summ <- rov.nav %>% 
  group_by(site_general,stratum) %>% 
  summarise(
    n.tx = n_distinct(dive_name),
    distance = sum(disp),
    area = sum(area)) %>% 
  arrange(site_general,desc(stratum))
...
lw_data <- filter(lw_data,to_use == 1) %>%
  ...
  arrange(sciName) %>% 
  filter(!is.na(a)) %>% 
  left_join(select(spp_codes,sciName,commonName)) %>% 
...
rov.boot <- rov.fish %>% 
  ...
  left_join(rov.nav.boot) %>% 
  mutate(
    abund = counts/area,
    biomass = weight_g/area) %>% 
  left_join(select(rov.logs,dive_name,site_general))

# Write to CSV
write_csv(rov.boot,here("Output/boot_data_rov.csv"))
...
rov.photo_obs  <- filter(rov.photo_obs,photo_id %in% rov.photo_info$photo_id) %>% 
  left_join(select(spp_codes,sci_name,sciName,commonName)) %>%
  filter(str_detect(sciName,"Sebastes|Sebastolobus|Sebastomus|Ophiodon|Merluccius")) %>%
  droplevels()
...
if (import.data) {
  if (sub.source == "DSN") {
    ...
    # Import CTD data
    sub.ctd <-  sqlFetch(sub.con, "tbl_Gear_Compare_Yok_ctd") %>% 
    ...
    # Import NAV data  
    sub.nav <-  sqlFetch(sub.con, "tbl_Gear_Compare_Yok_nav") %>% 
      rename(lat       = latitude,
             long      = longitude,
             cum_dist  = cum_dist_m) %>% 
      unite(date,year,month,day,sep = "-") %>% 
    ...
    sub.dens <- read_excel(here("Data/SUB/sub_data.xlsx"),
                           sheet = "Yok_Sub_Density_step3") %>% 
      rename(site_general = bank,
             dive_name    = dive_tx,
             sciName      = Scientific_Name,
             counts       = SumOfSumOffreq,
             distance     = SumOfdist,
             stratum      = depth_bin) %>% 
      mutate(site_general = str_replace(site_general,"footprint","The Footprint"),
             site_general = str_replace(site_general,"piggy","Piggy Bank"),
    ...
    sub.biom <- read_excel(here("Data/SUB/sub_data.xlsx"),
  ...
  # Save SUB data
  save(sub.dens,sub.biom,sub.ctd,sub.nav,file = here("Data/SUB/sub_data.Rdata"))
} else {
  # Load SUB data
...
# ROV data for bootstrap analysis 
sub.boot <- sub.biom %>% 
  group_by(site_general,dive_name,stratum,sciName) %>% 
  summarise(counts   = sum(counts),
            weight_g = sum(weight_g)) %>% 
  left_join(select(sub.biom,dive_name,distance,area)) %>% 
  mutate(abund   = counts/area,
         biomass = weight_g/area)
  
# Write to CSV
write_csv(sub.boot,here("Output/boot_data_sub.csv"))

# # Plot sub nav data by dive_name
# ggplot(sub.nav,aes(long,lat,group = dive_name)) +
#   geom_path(aes(colour = dive_name)) +
#   coord_map() +
#   theme_bw() +
...
# Tables  
**Table 1.** Length-weight table.

```{r LengthWeightTable}
# Get species in study
lw.spp <- unique(rov.boot$sciName)

# Format and print L/W table
if (doc.type == "docx") {
  regulartable(filter(lw_data,sciName %in% lw.spp)) %>% 
    italic(j = 1) %>%
  ...
  kable(filter(lw_data,sciName %in% lw.spp), 
        format = knitr.format,booktabs = T, escape = T,
        align = c("l","l","r","r","l","r","l","l"),
        digits = c(3)) %>% 
