...
pkgs <- c("tidyverse","knitr","swfscMisc","lubridate","here","boot",
...
load_pkgs(pkgs)
# Install and load patchwork package from github
if ("patchwork" %in% installed.packages()[ ,'Package'] == F) {
...
# ggplot(nav.sub,aes(long,lat,group = dive_name)) +
  ...
  for (i in unique(boot.data.rov$sciName)) {
    for (j in unique(boot.data.rov$site)) {
      for (k in unique(boot.data.rov$stratum)) {
        ...
        if (nrow(boot.data) > 0) {
          boot.abund <- as.numeric(rep(NA,B))
          boot.biom  <- as.numeric(rep(NA,B))
          for (ii in seq(1:B)) {
            boot.abund[ii] <- mean(sample(boot.data$dens_n,nrow(boot.data),replace = T))
            boot.biom[ii]  <- mean(sample(boot.data$dens_g,nrow(boot.data),replace = T))
          }
