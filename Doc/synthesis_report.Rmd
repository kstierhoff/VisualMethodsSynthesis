...
do.boot     <- T # Conduct bootstrap resampling
...
# Get site names
dive.sites.sub <- counts.sub %>% 
  select(site, stratum, dive_name) %>% 
  distinct()
...
obs.sub.boot <- obs.sub.pos %>% 
  ...
  filter(str_detect(sciName,filter.spp)) %>% 
  left_join(select(spp, sciName, commonName)) %>% 
  left_join(dive.sites.sub)

# SUB data for bootstrap analysis 
boot.data.sub <- obs.sub.boot %>% 
  ...
  select(site, everything(), -commonName)
...
# basic.df.rov <- boot.data.rov %>% 
#   group_by(sciName, site, stratum) %>% 
#   summarize(
