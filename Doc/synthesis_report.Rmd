...
comp.spp <- obs.all.summ %>% 
 filter(nGrand > 20 & !sciName %in% c("Sebastes sp.","Sebastomus sp.","Sebastolobus sp.","Merluccius productus"))
```
...
lw.hist.select <- ggplot(filter(lw.data.all, sciName %in% main.spp), aes(length_cm, fill = platform)) +
  geom_histogram(breaks = seq(0,70,10),colour = "white",alpha = 0.5) +
  scale_fill_manual(name = "Vehicle", values = c("ROV" = rov.colour, "SUB" = sub.colour)) + theme_bw() +
...
The length composition of observed fishes varied by survey platform. Similar
